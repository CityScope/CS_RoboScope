...
void main(){
  ...
  //float depth = LinearizeDepth(frag_z) / far; // divide by far for demonstration
  //out_frag = vec4(vec3(depth), 1.0);
  //out_frag = vec4(frag_z, frag_z, frag_z, 1);

 // out_frag = sum_color / sum_weights;

  //float depth = LinearizeDepth(frag_geom.w) / far; // divide by far for demonstration
  //out_frag = vec4(vec3(depth), 1.0);

  out_frag = mix(nearColor, farColor, smoothstep(near, far, frag_geom.w));
  //float focus = frag_z - focus_z;
  //focus = abs(dz_frag_focus);
  //focus = frag_z - focus_z;
 // float zdelta = clamp(focus, 0.0, 1.0);

 //working
  //out_frag = mix(nearColor, farColor, smoothstep(near, far, frag_z));
  // out_frag = vec4(frag_z, frag_z, frag_z, 1);
