...
public class DepthField {
  ...
  public DepthField(DwPixelFlow context) {
    this.context = context;
    String path = sketchPath();
    this.shader = context.createShader(path+"/data/depth_of_field.frag");
  }
  ...
  public void apply(PGraphicsOpenGL src, PGraphicsOpenGL dst, DwScreenSpaceGeometryBuffer geom) {
    ...
    Texture tex_geom = geom.pg_geom.getTexture();
    if (!tex_geom.available())  return;
