...
public void displayScene(PGraphicsOpenGL canvas) {
  ...
  if (drawMode == 0) {
    canvas.shape(volpeMapped);
  } else if (drawMode == 1) {
    canvas.shape(volpeOnly);
  } else if (drawMode == 2) {
    canvas.shape(volpeOnly);
  } else if (drawMode == 3) {
    canvas.shape(volepeOnlyBase);
  } else if (drawMode == 4) {
    canvas.shape(volepeOnlyBase);
    ...
    /*
     ...
     float transX [] = {0, 27.5, 27.5, 27.5};
     for (int j = 0; j < 3; j++) {
     pushMatrix();
     for (int i = 0; i < buildingsA.length; i++) {
     
     buildingsA[i].translate(0, transX[j], 0);
     canvas.shape( buildingsA[i]);
     }
     popMatrix();
     }
     */
