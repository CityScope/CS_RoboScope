...
void InterfacePanel::init() {
  ...
  Serial.println("Setting up Interface Mux:");

  int muxCounter = 0;
  while (muxCounter != 3) {
    if (!sx.begin(SX1509_ADDRESS_01) )
    {
  ...
  for (int i = 0; i < MOTORS_PER_PANEL; i++) {
    sx.pinMode(pushPin[i], INPUT_PULLUP);
    sx.pinMode(limitSwithPin[i], INPUT_PULLUP);
  }

  // FALLING, RISING, or CHANGE. Set it to falling, which will
  // mean the button was pressed:
  for (int i = 0; i < MOTORS_PER_PANEL; i++) {
    sx.enableInterrupt(pushPin[i], CHANGE  );
    sx.enableInterrupt(limitSwithPin[i], CHANGE  );
  }
  ...
  //debouncePin
  for (int i = 0; i < MOTORS_PER_PANEL; i++) {
    sx.debouncePin(pushPin[i]);
    sx.debouncePin(limitSwithPin[i]);
  }
...
void InterfacePanel::updateLimitState() {
  ...
  unsigned int intStatus = sx.interruptSource();
...
int InterfacePanel::getPushCurrentState(int i) {
  return  sx.digitalRead(pushPin[i]);
}


//---------------------------------------------------------------------------
int InterfacePanel::getLimitCurrentState(int i) {
  return  sx.digitalRead(limitSwithPin[i]);
}
